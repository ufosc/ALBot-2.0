import {
  ...
  let refreshRoles = async function (
  ...
  const GiveRole: ICommand = {
    ...
    execute: async (interaction: BaseCommandInteraction) => {
      ...
      if (
        (roles.has(name as string) && permitted.has(name as string)) ||
        (member.permissions.has("ADMINISTRATOR") && roles.has(name as string))
      ) {
        const role = await guild.roles.cache.find(
          (r: any) => r.name === (name as string)
        );
      ...
      if (member.permissions.has("ADMINISTRATOR") && roles.has(name as string)) {
        ...
        if (!role.permissions.has("ADMINISTRATOR")) {
          ...
          await interaction.reply(
            `Cannot open role with administrator privileges`
          );
        }
      } else {
        await interaction.reply(
          `You are not an admin or something else went wrong`
        );
      }
    },
  };
    ...
    execute: async (interaction: BaseCommandInteraction) => {
      ...
      if (member.permissions.has("ADMINISTRATOR") && roles.has(name as string)) {
        if (permitted.has(name as string)) {
          ...
          await interaction.reply(
            `either role does not exist or is already open!`
          );
      } else {
        await interaction.reply(
          `You are not an admin or something else went wrong`
  ...
  const ShowRoles: ICommand = {
    ...
    execute: async (interaction: BaseCommandInteraction) => {
      ...
      roles.forEach((role) =>
        messageLines.push(`${role}${permitted.has(role) ? "*" : ""}`)
      );
